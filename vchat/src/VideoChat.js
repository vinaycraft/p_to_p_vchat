...
function localDevWsUrl(pageIsHttps) {
...
function VideoChat() {
  ...
  useEffect(() => {
    document.title = roomId ? `Video Chat · ${roomId}` : 'Video Chat';
  }, [roomId]);
      ...
      {showWaitingOverlay && (
        <div className="waiting-overlay" aria-live="polite">
          ...
          <p className="waiting-title">
            {isQueueMode ? 'Finding someone…' : (uiPhase === 'peer-left' ? 'Waiting for someone' : 'Share link to start')}
          </p>
          {!isQueueMode && (
            <>
